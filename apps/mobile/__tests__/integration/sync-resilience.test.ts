...
import { getTestDatabase, closeDatabase, getDatabase } from '@/lib/db/local-database';
    ...
    it('should mark items as failed on server validation error', async () => {
        ...
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
            ...
            json: async () => ({
                success: true,
                results: {
                    itemResults: [{ id: newTask.id, status: 'failed', error: 'Database constraint failed' }]
                }
            })
        }));
        ...
        const syncItem = await db.getFirstAsync<{ status: string; error: string | null }>(
    ...
    it('should handle duplicate ID conflict', async () => {
        ...
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
            ...
            json: async () => ({
                success: true,
                results: {
                    itemResults: [{ id: newTask.id, status: 'failed', error: 'Conflict: ID already exists' }]
                }
            })
        }));
