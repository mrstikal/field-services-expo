...
export default function ReportDetailScreen() {
  ...
  const getFieldValue = (field: FormField, formData: Record<string, unknown>): string => {
    ...
    switch (field.type) {
      case 'checkbox':
        return value ? 'Yes' : 'No';
      case 'select': {
        const option = field.options?.find((o: { value: string | number }) => o.value === value);
  ...
  const formatFormData = (formData: Record<string, unknown>, category: TaskCategory) => {
    ...
    return (
      <View className="flex-1 bg-slate-50">
        ...
        >
          ...
          <Text className="text-lg font-semibold text-gray-800">Report Detail</Text>
          <View className="w-10" />
        </View>
    ...
    return (
      <View className="flex-1 bg-slate-50">
        ...
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-3 text-base text-red-500">Failed to load report.</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-slate-50">
      ...
      <ScrollView className="flex-1 p-4">
        {/* Report Header */}
        <Text className="mb-3 text-xl font-semibold text-gray-800">Report #{report.id.slice(0, 8)}</Text>

        {/* Task Information */}
        <View className="mb-4 rounded-lg border border-gray-200 bg-white p-4">
          <Text className="mb-2 text-sm font-semibold uppercase text-gray-500">Task Information</Text>
          <View className="mb-2">
          ...
          <View className="mb-2">
            ...
            <Text className="text-sm font-medium text-gray-800 capitalize">{task.category}</Text>
          </View>
          <View className="mb-2">
            <Text className="text-xs text-gray-500">Customer</Text>
            <Text className="text-sm font-medium text-gray-800">{task.customer_name}</Text>
          </View>
          <View>
            <Text className="text-xs text-gray-500">Phone</Text>
            <Text className="text-sm font-medium text-gray-800">{task.customer_phone}</Text>
          </View>
        </View>
        ...
        <View className="mb-3 rounded-lg border border-gray-200 bg-white p-3">
          ...
          <Text className="text-sm font-medium text-gray-800 capitalize">{report.status}</Text>
        </View>

        {/* Form Data */}
        {formData.length > 0 && (
          <View className="mb-4 rounded-lg border border-gray-200 bg-white p-4">
            <Text className="mb-3 text-sm font-semibold uppercase text-gray-500">Report Details</Text>
         ...
         {report.photos && report.photos.length > 0 && (
           <View className="mb-4 rounded-lg border border-gray-200 bg-white p-4">
             <Text className="mb-3 text-sm font-semibold uppercase text-gray-500">Photo Documentation</Text>
        ...
        {report.signature && (
          <View className="mb-4 rounded-lg border border-gray-200 bg-white p-4">
            <Text className="mb-3 text-sm font-semibold uppercase text-gray-500">Customer Signature</Text>
            <View className="h-[150px] w-full overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
              <Image
                source={{ uri: report.signature.startsWith('http') ? report.signature : `data:image/png;base64,${report.signature}` }}
                className="h-full w-full"
                resizeMode="contain"
              />
            </View>
        ...
        <View className="flex-row gap-3">
          <View className="flex-1 rounded-lg border border-gray-200 bg-white p-3">
            <Text className="mb-1 text-xs font-semibold uppercase text-gray-500">Created at</Text>
            <Text className="text-sm font-medium text-gray-800">{new Date(report.created_at).toLocaleString()}</Text>
          </View>
          <View className="flex-1 rounded-lg border border-gray-200 bg-white p-3">
            <Text className="mb-1 text-xs font-semibold uppercase text-gray-500">Updated at</Text>
            <Text className="text-sm font-medium text-gray-800">{new Date(report.updated_at).toLocaleString()}</Text>
          </View>
        </View>
      </ScrollView>
