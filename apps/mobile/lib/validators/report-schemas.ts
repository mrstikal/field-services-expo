...
const requiredTextValidator = (min?: number, max?: number) =>
  z.preprocess(
    value => {
    ...
    textValidator(min, max)
  );

const optionalTextValidator = (min?: number, max?: number) =>
  z.preprocess(
    value => {
      if (value === null || value === undefined) {
        return undefined;
    ...
    z
      .string()
      .min(min ?? 0, min ? `Minimum ${min} characters` : undefined)
      .max(
        max ?? Number.POSITIVE_INFINITY,
        max ? `Maximum ${max} characters` : undefined
      )
      .optional()
  );
...
export function createReportSchema(category: TaskCategory) {
  ...
  for (const field of template.fields) {
    ...
    switch (field.type) {
      case 'text':
        schema = field.required
          ? requiredTextValidator(field.minLength, field.maxLength)
          : optionalTextValidator(field.minLength, field.maxLength);
        break;
      case 'number':
    ...
    fieldSchemas[field.id] = field.required ? schema : schema.optional();
  }
