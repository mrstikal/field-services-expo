...
import { generateReportHTML, generatePDF, sharePDF, savePDF, deletePDF } from '../pdf-generator';
...
describe('generateReportHTML', () => {
  ...
  it('should generate PDF and return URI', async () => {
    ...
    vi.mocked(Print.printToFileAsync).mockResolvedValue({ uri: mockUri } as never);
  ...
  it('should throw error if PDF generation fails', async () => {
    ...
    vi.mocked(Print.printToFileAsync).mockRejectedValue(new Error('Print failed'));
  ...
  it('should throw error if save fails', async () => {
    ...
    vi.mocked(FileSystem.copyAsync).mockRejectedValue(new Error('Copy failed'));

    await expect(savePDF(uri, filename)).rejects.toThrow('Failed to save PDF');
  });
});
  ...
  it('should handle error silently', async () => {
    ...
    vi.mocked(FileSystem.deleteAsync).mockRejectedValue(new Error('Delete failed'));
