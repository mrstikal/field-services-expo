...
export const useReportStore = create<ReportState>(set => ({
  ...
  removePhoto: (id: string) =>
    set(state => ({
      currentReport: {
        ...state.currentReport,
        photos: state.currentReport.photos.filter(
          (_, index) => index !== parseInt(id)
        ),
      },
    })),

  setFormData: (data: ReportFormData) =>
    set(state => ({
      currentReport: {
        ...state.currentReport,
  ...
  addLocalReport: (report: Report) =>
    set(state => ({
      localReports: [...state.localReports, report],
    })),

  updateLocalReport: (reportId: string, updates: Partial<Report>) =>
    set(state => ({
  ...
  removeLocalReport: (reportId: string) =>
    set(state => ({
