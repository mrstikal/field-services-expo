        ...
        return (
          <Controller
            ...
            render={({ field: fieldProps, fieldState: { error: fieldError } }) => (
              <View className="rounded-lg border border-gray-200 bg-white">
                <TouchableOpacity
                  className={`flex-row items-center justify-between p-3 ${fieldError ? 'border border-red-500' : ''}`}
                  onPress={() => {}} // Will be implemented in next step
                >
                  <Text className="text-sm text-gray-500">
                    {fieldProps.value ? field.options?.find(o => o.value === fieldProps.value)?.label || '' : 'Select...'}
                  </Text>
