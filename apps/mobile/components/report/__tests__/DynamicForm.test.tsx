...
import { DynamicForm, DynamicFormHandle } from '../DynamicForm';
...
vi.mock('../FormField', () => ({
  FormField: vi.fn(({ control, field }) => (
    <input
      ...
      onChange={(e) => {
        control._fields[field.id]?._f.onChange({ target: { value: e.target.value } });
      }}
    />
  )),
}));
...
describe('DynamicForm', () => {
  ...
  it('should show/hide fields based on conditional logic', () => {
    ...
    fireEvent(getByTestId('form-field-field1'), 'change', { target: { value: 'show' } });
  ...
  it('should call onSubmit with form data on successful submission', async () => {
    ...
    const { getByTestId } = render(
      <DynamicForm ref={formRef} template={mockTemplate} onSubmit={onSubmitMock} />
    );

    fireEvent(getByTestId('form-field-field1'), 'change', { target: { value: 'test value' } });
    fireEvent(getByTestId('form-field-requiredField'), 'change', { target: { value: 'some data' } });
  ...
  it('should show alert on validation failure', async () => {
    ...
    fireEvent(getByTestId('form-field-requiredField'), 'change', { target: { value: 'error' } });
  ...
  it('should return form data using getFormData', () => {
    ...
    fireEvent(getByTestId('form-field-field1'), 'change', { target: { value: 'data for get' } });
    fireEvent(getByTestId('form-field-requiredField'), 'change', { target: { value: 'more data' } });
  ...
  it('should reset form data using resetForm', () => {
    ...
    fireEvent(getByTestId('form-field-field1'), 'change', { target: { value: 'initial value' } });
