...
vi.mock('@/components/ui/select', () => ({
  ...
  SelectItem: vi.fn(({ children, value }: any) => (
...
vi.mock('@/components/ui/label', () => ({
  Label: vi.fn(({ children, htmlFor }: any) => (
...
describe('TaskForm', () => {
  ...
  const mockUseFormReturn = {
    register: vi.fn(() => ({})),
    handleSubmit: vi.fn((cb: any) => (e: React.FormEvent) => {
      e.preventDefault();
      cb({});
    }),
    control: {},
  ...
  it('should render all form fields', () => {
    ...
    expect(
      screen.getByLabelText('Estimated Time (minutes) *')
    ).toBeInTheDocument();
  ...
  it('should initialize with task values when task is provided', () => {
    ...
    render(
      <TaskForm task={mockTask} onSubmit={mockSubmit} onCancel={mockCancel} />
    );
  ...
  it('should fetch technicians if not provided via props', () => {
    ...
    expect(
      screen.getByText('Tech One (tech1@example.com)')
    ).toBeInTheDocument();
  });
  ...
  it('should disable submit button when loading is true', () => {
    render(
      <TaskForm onSubmit={mockSubmit} onCancel={mockCancel} loading={true} />
    );
